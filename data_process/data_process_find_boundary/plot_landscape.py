...
if robot_option == 1:
    ...
    dir2 = file_dir+'dairlib_data/find_boundary/' + robot + '2D_rom/4D_task_space/' + 'robot_' + str(robot_option) + \
           '_2d_initial_model/'
    dir3 = file_dir+'dairlib_data/find_boundary/' + robot + '2D_rom/4D_task_space/' + 'robot_' + str(robot_option) + \
           '_large_iter100/'
    dir4 = file_dir + 'dairlib_data/find_boundary/' + robot + '2D_rom/4D_task_space/' + 'robot_' + str(robot_option) + \
           '_small_iter200/'
...
def process_data_from_direction(i, dir1):
    ...
    for j in range(data_dir.shape[0]):
        gamma = np.genfromtxt(dir1 + str(int(data_dir[j, 0])) + '_' + str(0) + '_gamma.csv', delimiter=",")
        cost = data_dir[j, 1]
        if cost < 35:
